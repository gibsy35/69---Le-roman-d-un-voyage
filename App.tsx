...
import { BookOpen, Compass, ShieldCheck, Mail, Map, Star, Heart, ArrowRight, DollarSign, Calendar, Info, HelpCircle, Lock } from 'lucide-react';
...
export default function App() {
  ...
  // Authentication for Patrice Lequime's private Intranet
  const [isAuthenticated, setIsAuthenticated] = useState<boolean>(() => {
    return localStorage.getItem('patrice_auth') === 'true' || sessionStorage.getItem('patrice_auth') === 'true';
  });
  const [isAuthModalOpen, setIsAuthModalOpen] = useState<boolean>(false);

  // Navigation interceptor to protect the private Intranet
  const handleSetView = (view: 'store' | 'timeline' | 'intranet') => {
    if (view === 'intranet' && !isAuthenticated) {
  ...
  const handleLogout = () => {
    ...
    sessionStorage.removeItem('patrice_auth');
    localStorage.removeItem('patrice_auth');
    setView('store');
  };
  ...
  return (
    <div className="bg-[#FCFAF6] min-h-screen text-[#4A3225] flex flex-col font-sans selection:bg-[#EBDCCB]">
      ...
      <Header currentView={currentView} setView={handleSetView} ordersCount={pendingOrders} />
        ...
        <AnimatePresence mode="wait">
          {currentView === 'store' && (
            ...
            >
              ...
              <BookDetail onSuccessOrder={fetchOrdersList} onOpenAuthModal={() => setIsAuthModalOpen(true)} />
            </motion.div>
          )}
      ...
      {/* Auth Modal for Patrice Lequime */}
      <AuthModal
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="space-y-3 text-xs font-mono">
              ...
              <ul className="space-y-2">
                <li>
                  <button onClick={() => handleSetView('store')} className="text-[#EBDCCB]/80 hover:text-white transition-colors bg-transparent border-none text-left p-0 cursor-pointer">
                    → Présentation & Boutique du Livre
                  </button>
                </li>
                <li>
                  <button onClick={() => handleSetView('timeline')} className="text-[#EBDCCB]/80 hover:text-white transition-colors bg-transparent border-none text-left p-0 cursor-pointer">
                    → Journal de Bord interactif (22 étapes)
                  </button>
                </li>
                <li>
                  <button onClick={() => handleSetView('intranet')} className="text-[#EBDCCB]/80 hover:text-white transition-colors bg-transparent border-none text-left p-0 cursor-pointer">
                    → Accès Administrateur (Intranet Auteur)
                  </button>
                </li>
              </ul>
            </div>
